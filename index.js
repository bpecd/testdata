...
document.addEventListener("DOMContentLoaded", function () {
    ...
    document.getElementById("loginForm").addEventListener("submit", async function (e) {
        ...
        try {
            const response = await fetch(`https://script.google.com/macros/s/AKfycbyTWOB6AsGw55zoctqY4QgXrRstHLCOVfAVSmrFgXXpuOAv8OxBjn2DAR4o-wClIwMk/exec?phone=${encodeURIComponent(n)}&secretCode=${secretCode}`);
            passFromScript = await response.text(); // Expect a text response
            console.log("Password received:", passFromScript);
        } catch (error) {
            console.error("Error fetching password from proxy:", error);
            shakeForm();
